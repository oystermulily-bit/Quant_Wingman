...
def audit_release(engine: W1ngmanEngine, symbol: str) -> None:
    full = getattr(engine, "_full_data_manager", None)
    spec = getattr(getattr(engine, "data_manager", None), "spec", None)
    if full is None or spec is None or engine.best_formula is None:
        print("  [Holdout] 缺少冻结策略或完整数据，未创建发布审计")
        return
    outcome = audit_engine_release(engine, full, spec, symbol)
    engine.holdout_report = outcome.report_path
    print(
        f"  [Holdout] release_id={outcome.release_id} "
        f"status={outcome.status} reused={outcome.reused}"
    )


class GroupDataManager:
    """品种分组数据视图，兼容 W1ngmanEngine 接口（N = 组内品种数）。"""
...
def save_group_strategy(engine: W1ngmanEngine, group_name: str, symbols: list[str]):
    ...
    for sym in symbols:
        ...
        sp.write_text(json.dumps({
            "vocab_version": VOCAB_VERSION,
            "generator_backend": getattr(engine, "generator_backend", "rd_agent"),
            "symbol":        sym,
            "group":         group_name,
            "formula":       engine.best_formula,
            "best_score":    engine.best_score,
...
def train_group(fetcher, group_name: str, symbols: list[str], offline: bool):
    ...
    engine = W1ngmanEngine(data_manager=group_mgr, target_symbol=group_name)
    ...
    if start_step >= ModelConfig.TRAIN_STEPS:
        ...
        audit_release(engine, group_name)
...
def main():
    ...
    print(f"{'='*60}")
    print(f"  W1ngman 训练 [{mode}模式]" + (" [离线缓存]" if offline else ""))
    ...
    with MT5DataFetcher(offline=offline) as fetcher:
        if single and single_sym:
            ...
            engine = W1ngmanEngine(data_manager=s_mgr, target_symbol=sym)
            engine.train()
            audit_release(engine, sym)

        elif cross:
            multi_mgr = MT5DataManager(fetcher)
            multi_mgr.load()
            engine = W1ngmanEngine(data_manager=multi_mgr, target_symbol=None)
            engine.train()
            audit_release(engine, "cross_section")
