...
class StartTrainingRequest(BaseModel):
    ...
    # Required on purpose: stale cached JavaScript must fail loudly instead of
    # silently falling back to the old 30-round default.
...
def health() -> dict[str, str]:
    return {
        "status": "ok",
        "version": "1.3.0-rd-custom-rounds",
...
def api_config() -> dict[str, Any]:
    ...
    return {
        ...
        "generator_model": ModelConfig.RD_AGENT_MODEL,
...
def api_overview() -> dict[str, Any]:
    ...
    if job and job.get("symbol") and active:
        ...
        row = _progress_with_live_step(
            sym,
            active=True,
            train_steps_override=int(job.get("rounds") or ModelConfig.TRAIN_STEPS),
        )
        progress = {
            "symbol": row["symbol"],
            "status": "running_job",
...
def api_realtime_feishu_test(req: FeishuTestRequest) -> dict[str, Any]:
    ...
    ok, msg = send_text(
        "✅ quant_w1ngman 飞书通知测试：配置正常。信号方向转折时会推送提醒。",
        webhook_url=url,
        secret=secret or "",
    )
    if not ok:
