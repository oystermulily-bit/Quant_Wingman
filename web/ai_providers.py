...
Supports:
  ...
  - siliconflow: OpenAI-compatible API @ https://api.siliconflow.cn/v1 (user API key)
...
PROVIDERS = ("deepseek", "siliconflow", "openclaw", "openclaw_wb")
...
def _probe_qclaw_gateway(base_url: str, token: str, *, timeout: float = 5.0) -> bool:
    ...
    headers = {"Authorization": f"Bearer {token}", "User-Agent": "quant_w1ngman"}
...
def provider_status() -> dict[str, Any]:
    ...
    return {
        "providers": [
            ...
            {
                "id": "siliconflow",
                "label": f"硅基流动 ({SILICONFLOW_MODEL})",
                "available": True,
                "needs_user_key": True,
                "hint": f"{SILICONFLOW_MODEL} · {SILICONFLOW_BASE_URL}",
            },
            {
                "id": "openclaw",
    ...
    *,
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "quant_w1ngman-AI-Analyze",
        },
    )
    try:
        with urllib.request.urlopen(req, timeout=timeout) as resp:
