...
用户正在用 quant_w1ngman / RD-Agent 研发可解释因子公式（token 序列，由特征与算子组成）。
...
- 可讨论：验证分数走势、最优分数是否停滞、候选公式质量、多样性、有效率和超时率。
...
def _training_curve(history: dict[str, Any], max_points: int = 500) -> dict[str, Any]:
    ...
    total = len(steps)
    keys = (
        "step", "best_score", "val_score", "avg_reward", "rd_candidates",
        "rd_valid", "rd_timeouts", "round_seconds", "generator_backend",
    )
...
def _history_summary(history: dict[str, Any]) -> dict[str, Any]:
    ...
    rd_candidates = history.get("rd_candidates") or []
    rd_valid = history.get("rd_valid") or []
    rd_timeouts = history.get("rd_timeouts") or []
    round_seconds = history.get("round_seconds") or []
    steps = history.get("step") or []
    ...
    if rd_candidates:
        summary["rd_candidates_last"] = rd_candidates[-1]
    if rd_valid:
        summary["rd_valid_last"] = rd_valid[-1]
        summary["rd_valid_total"] = sum(rd_valid)
    if rd_timeouts:
        summary["rd_timeouts_last"] = rd_timeouts[-1]
        summary["rd_timeouts_total"] = sum(rd_timeouts)
    if round_seconds:
        summary["round_seconds_last"] = round_seconds[-1]
        summary["round_seconds_mean"] = sum(round_seconds) / len(round_seconds)
    return summary
