const API = "";
const LEGACY_PROJECT_BRAND = ["Alpha", "Master"].join("");
const LEGACY_MODEL_BRAND = ["Alpha", "GPT"].join("");

function brandSafeText(value) {
  return String(value ?? "")
    .split(LEGACY_PROJECT_BRAND).join("quant_w1ngman")
    .split(LEGACY_PROJECT_BRAND.toLowerCase()).join("quant_w1ngman")
    .split(LEGACY_MODEL_BRAND).join("W1ngman")
    .split(LEGACY_MODEL_BRAND.toLowerCase()).join("w1ngman");
}
...
function renderDebugView(serverLines = [], errorLines = []) {
  ...
  const next = brandSafeText(parts.length ? parts.join("\n") : emptyDebugMessage());
...
function renderDataFileCard(info) {
  ...
  if (info.valid === false) {
    const safeDataFile = brandSafeText(info.data_file);
    card.className = "data-file-card invalid";
    card.innerHTML = `
      <div class="data-file-error">${info.message || "文件无效"}</div>
      <div class="data-file-path">${safeDataFile}</div>
    `;
    startBtn.disabled = true;
    if ($("retrainBtn")) $("retrainBtn").disabled = true;
    if ($("exportTrainingBtn")) $("exportTrainingBtn").disabled = true;
  ...
  card.innerHTML = `
    ...
    <div class="path" title="${safeDataFile}">${info.filename || safeDataFile}</div>
  `;
...
function renderStrategyFileCard(info) {
  ...
  if (info.valid === false) {
    const safeStrategyFile = brandSafeText(info.strategy_file);
    card.className = "data-file-card invalid";
    card.innerHTML = `
      <div class="data-file-error">${info.message || "文件无效"}</div>
      <div class="data-file-path">${safeStrategyFile}</div>
    `;
    selectedStrategyFile = null;
    selectedStrategySymbol = null;
    updateBtStartBtn();
  ...
  const safeStrategyFile = brandSafeText(info.strategy_file);
  const safeDataPath = brandSafeText(dataPath);
  const dataHint = dataPath
    ? (dataOk ? safeDataPath : `（文件不存在）${safeDataPath}`)
  ...
  card.innerHTML = `
    ...
    <div class="path" title="${safeStrategyFile}">策略: ${info.filename || safeStrategyFile}</div>
    <div class="path ${dataPath && dataOk ? "" : "data-file-missing"}" title="${safeDataPath}">数据: ${dataHint}</div>
  `;
  updateBtStartBtn();
}
...
const CHART_SERIES = [
  { key: "best_score", label: "最优分数", borderColor: "#a78bfa" },
  { key: "val_score", label: "验证分数", borderColor: "#c4b5fd" },
];

function drawTrainingChartCanvas(canvas, steps, history) {
  const rows = CHART_SERIES.map((series) => ({
    ...series,
    values: (history?.[series.key] || []).map(Number),
  })).filter((series) => series.values.some(Number.isFinite));
  if (!rows.length) return;

  const rect = canvas.parentElement.getBoundingClientRect();
  const width = Math.max(560, Math.floor(rect.width || 900));
  const height = Math.max(260, Math.floor(rect.height || 288));
  ...
  const span = Math.max(max - min, Math.abs(max) * 0.08, 0.1);
  min -= span * 0.12;
  max += span * 0.12;

  const pad = { left: 58, right: 18, top: 38, bottom: 30 };
  const plotWidth = width - pad.left - pad.right;
  const plotHeight = height - pad.top - pad.bottom;
  const x = (index) => pad.left + (index / Math.max(1, steps.length - 1)) * plotWidth;
  ...
  rows.forEach((row, rowIndex) => {
    ...
    ctx.lineJoin = "round";
    ctx.lineCap = "round";
    ctx.shadowColor = row.borderColor;
    ctx.shadowBlur = 7;
    ctx.beginPath();
    let started = false;
    ...
    const legendX = pad.left + rowIndex * 128;
    ctx.fillStyle = row.borderColor;
    ctx.beginPath();
    ctx.arc(legendX, 17, 4, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = "#a9bccf";
    ctx.textAlign = "left";
    ctx.fillText(row.label, legendX + 10, 17);
  });

  ctx.fillStyle = "#6b7d92";
  ctx.textBaseline = "top";
  ctx.textAlign = "left";
  ctx.fillText(String(Number(steps[0]) + 1), pad.left, height - pad.bottom + 9);
  ctx.textAlign = "right";
  ctx.fillText(String(Number(steps[steps.length - 1]) + 1), width - pad.right, height - pad.bottom + 9);
}
...
function renderChart(history, label, progress) {
  ...
  if (!steps.length) {
    ...
    if (progress?.current_step > 0) {
      $("chartHint").textContent = `研发中 第 ${progress.current_step}/${progress.train_steps} 轮，曲线每轮更新`;
    } else {
  ...
  drawTrainingChartCanvas(canvas, steps, history);
  chartSymbol = label;
...
async function loadSymbolChart(symbol, progress) {
  ...
  try {
    ...
    const displayProgress = { ...(progress || {}), ...data };
    updateFileProgress(displayProgress);
    renderChart(data.history, symbol, displayProgress);
...
function symbolIdentity(value) {
  return String(value || "")
    ...
    .replace(/\.(SH|SZ|BJ)$/, "")
    .split("_")
    .filter((part) => !["QFQ", "HFQ", "BFQ", "FORWARD", "BACKWARD"].includes(part))
    .join("_");
}

function resolveCurveSymbol(symbol, strategies) {
  if (!symbol) return null;
  const rows = Array.isArray(strategies) ? strategies : [];
  if (rows.some((row) => row.symbol === symbol)) return symbol;
  const identity = symbolIdentity(symbol);
  const matches = rows.filter((row) => symbolIdentity(row.symbol) === identity);
  return matches.length === 1 ? matches[0].symbol : symbol;
}
...
function updateTrainingUI(training, progress) {
  ...
  const roundsInput = $("trainRoundsInput");
  // Only mirror the server-confirmed value while a job is running. Once it
  // finishes, keep the user's next input instead of restoring the old target.
  if (roundsInput && active && job?.rounds != null) {
    roundsInput.value = String(job.rounds);
  }
  if (roundsInput) roundsInput.disabled = !!active;
  ...
  const backend = job.generator_backend === "rd_agent" ? "RD-Agent" : "后端异常";
  $("logHint").textContent = `${backend} · ${job.rounds ?? "?"} 轮 · ${job.log_path || "—"}`;
  updateTrainingTimeFields(progress, training);

  const logView = $("logView");
  const atBottom = isViewAtBottom(logView);
  logView.textContent = brandSafeText((training.log_tail || []).join("\n") || "等待输出…");
...
async function refreshOverview() {
  ...
  const rawSymbol = overview.progress?.symbol || overview.data_file?.symbol || training?.job?.symbol || selectedSymbol;
  const sym = resolveCurveSymbol(rawSymbol, strategies.strategies);
  if (overview.data_file) {
    renderDataFileCard(sym ? { ...overview.data_file, symbol: sym } : overview.data_file);
  }
...
async function loadConfig() {
  ...
  if (health.custom_rounds_required !== "true") {
    throw new Error("网页后端仍是旧版本，请重启服务后按 Ctrl+F5 刷新页面");
  }

  const cfg = await fetchJSON("/api/config");
  const roundsInput = $("trainRoundsInput");
  if (roundsInput && !roundsInput.dataset.initialized) {
    roundsInput.value = String(cfg.train_steps || 30);
    roundsInput.dataset.initialized = "1";
    roundsInput.addEventListener("input", () => roundsInput.setCustomValidity(""));
  }
  debugMode = !!cfg.debug_mode;
  $("debugModeCheck").checked = debugMode;
  if (cfg.generator_backend !== "rd_agent" || cfg.reinforce_enabled !== false) {
    throw new Error("服务器不是 RD-Agent-only 版本，请关闭旧网页服务后重新启动当前项目");
  }
  const modelName = String(cfg.generator_model || "DeepSeek").split("/").pop();
  $("deviceMeta").textContent = `RD-Agent · ${modelName} · 默认 ${cfg.train_steps} 轮 · ${cfg.device}`;
  if (cfg.error_log) {
...
async function initAiPanel(cfg) {
  ...
  if (!keyInput.dataset.aiStatusBound) {
    ...
    providerSelect?.addEventListener("change", updateAiChannelHint);
  }
}
...
function resolveAiFromKey(raw) {
  ...
  const selected = $("aiProviderSelect")?.value || "deepseek";
  return { provider: selected, apiKey: (raw || "").trim(), isAlias: false };
}
...
function updateAiChannelHint() {
  ...
  } else if (resolved.provider === "siliconflow") {
    if (headHint) headHint.textContent = "硅基流动 · DeepSeek-V4-Pro";
    hint.textContent = row?.hint || "当前：硅基流动（OpenAI 兼容 API）。";
...
function readTrainingRounds() {
  const input = $("trainRoundsInput");
  const rounds = Number(input?.value);
  if (!Number.isInteger(rounds) || rounds < 1 || rounds > 10000) {
    const message = "请输入 1 到 10000 之间的整数轮数";
    if (input) {
...
async function startTraining() {
  ...
  try {
    const rounds = readTrainingRounds();
    const res = await fetchJSON("/api/training/start", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ data_file: selectedDataFile, from_scratch: false, rounds }),
    });
    if (res.job?.generator_backend !== "rd_agent") {
      throw new Error("训练启动已中止：后端未确认使用 RD-Agent");
    }
    if (Number(res.job?.rounds) !== rounds) {
      throw new Error(`训练启动已中止：请求 ${rounds} 轮，后端确认值为 ${res.job?.rounds ?? "缺失"}`);
    }
...
async function retrainFromScratch() {
  ...
  try {
    ...
    const res = await fetchJSON("/api/training/start", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ data_file: selectedDataFile, from_scratch: true, rounds }),
    });
    if (res.job?.generator_backend !== "rd_agent") {
      throw new Error("重新训练已中止：后端未确认使用 RD-Agent");
    }
    if (Number(res.job?.rounds) !== rounds) {
      throw new Error(`重新训练已中止：请求 ${rounds} 轮，后端确认值为 ${res.job?.rounds ?? "缺失"}`);
    }
    selectedSymbol = res.data_file?.symbol || res.job?.symbol;
    renderDataFileCard(res.data_file);
    await refreshOverview();
...
function switchPage(page) {
  if (!["train", "backtest", "realtime"].includes(page)) return;
  currentPage = page;
...
async function refreshBacktest() {
  ...
  const logText = brandSafeText((st.log_tail || []).join("\n") || "等待任务…");
  if (logView) {
    const atBottom = isViewAtBottom(logView);
...
function sparklineSVG(values, { color = "#8b5cf6", fillRGB = null, w = 74, h = 22 } = {}) {
...
function renderPortfolio(report) {
  ...
  const rollSpark = eq ? sparklineSVG(eq.rolling_sharpe, { color: "#8b5cf6", fillRGB: "139, 92, 246" }) : "";
...
function verticalGradient(chart, rgb, topOpacity, bottomOpacity) {
  const { ctx, chartArea } = chart;
  if (!chartArea) return `rgba(${rgb}, ${topOpacity})`;
  const g = ctx.createLinearGradient(0, chartArea.top, 0, chartArea.bottom);
  g.addColorStop(0, `rgba(${rgb}, ${topOpacity})`);
  g.addColorStop(0.62, `rgba(${rgb}, ${(topOpacity + bottomOpacity) / 4})`);
  g.addColorStop(1, `rgba(${rgb}, ${bottomOpacity})`);
  return g;
}
...
const EQUITY_OPTIONS = {
  ...
  scales: {
    x: {
      ...
      grid: { color: "rgba(43,43,54,0.05)" },
      border: { color: "rgba(43,43,54,0.12)" },
    },
    y: {
