...
class RDFormulaGenerator:
    ...
    def _system_prompt(self) -> str:
        ...
        return (
            ...
            "Propose diverse quantitative factor hypotheses and expressions in reverse Polish notation (RPN). "
            "Use only the exact feature and operator names supplied below. Never emit Python, "
            "SQL, file paths, data requests, executable code, or unknown identifiers. There is no "
            "Coder or Co-STEER stage: the local StackVM alone executes formula tokens.\n"
            f"FEATURES: {features}\n"
            ...
            "Keep each hypothesis and rationale under 20 words. Return JSON only: "
            "{\"formulas\":[{\"tokens\":[\"RET\",\"TS_MEAN_5\"],"
            "\"hypothesis\":\"testable market hypothesis\","
    ...
    def _user_prompt(
        ...
        *,
        ...
        safe_context = self._sanitise_research_context(research_context)
        return json.dumps(
            {
                "round": round_index,
                "requested_formula_count": count,
                "previous_elites": safe_feedback,
                "research_guidance": safe_context,
                "instructions": (
    ...
    @staticmethod
    def _sanitise_research_context(context: dict[str, Any] | None) -> dict[str, Any]:
        """Allow research prose/token mutations only; reject raw numeric arrays."""
        if not isinstance(context, dict):
            return {}
        result: dict[str, Any] = {}
        for key in ("next_hypotheses", "repeated_failure_reasons"):
            rows = context.get(key, [])
            if isinstance(rows, list):
                result[key] = [str(item)[:300] for item in rows[:8]]
        mutations = context.get("formula_mutations", [])
        if isinstance(mutations, list):
            result["formula_mutations"] = [
                item for item in mutations[:8]
                if isinstance(item, dict) and set(item).issubset({"from", "to", "reason"})
            ]
        return result
