...
class VMExecutionResult:
    """VM values plus locations that were invalid before numerical cleaning."""
...
class StackVM:
    ...
    def _normalize_output(x: torch.Tensor) -> torch.Tensor:
        ...
        # Do not branch on statistics computed over the full sequence.  Even a
        # constant prefix must follow the same causal path after future rows
        # are appended.
    ...
    def execute_with_diagnostics(self, formula_tokens, feat_tensor):
        """Execute and preserve the invalid mask before numerical cleaning."""
        ...
        try:
            for token in formula_tokens:
                ...
                if token < self.feat_offset:
                    ...
                    stack.append((value, invalid))
                elif token in self.op_map:
                    arity = self.arity_map[token]
                    if len(stack) < arity:
                        return None
                    entries = [stack.pop() for _ in range(arity)]
                    entries.reverse()
                    res = self.op_map[token](*[value for value, _ in entries])
                    invalid = ~torch.isfinite(res)
                    for _, arg_invalid in entries:
                        invalid = invalid | arg_invalid
                    res = torch.nan_to_num(
                        res, nan=0.0, posinf=1.0, neginf=-1.0
                    )
                    stack.append((res, invalid))
                else:
                    return None
            if len(stack) != 1:
                return None
            result, invalid = stack[0]
            normalized = self._normalize_output(result)
            invalid = invalid | ~torch.isfinite(normalized)
            normalized = torch.nan_to_num(
                normalized, nan=0.0, posinf=3.0, neginf=-3.0
            )
            return VMExecutionResult(normalized, invalid)
        except Exception:
            return None

    def execute(self, formula_tokens, feat_tensor):
        """Backward-compatible value-only VM interface."""
        result = self.execute_with_diagnostics(formula_tokens, feat_tensor)
