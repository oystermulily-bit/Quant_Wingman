...
def _ema(x: torch.Tensor, smoothing_weight: float) -> torch.Tensor:
    """指数加权移动平均（因果）。smoothing_weight 越大越关注近期。"""
    # 用递推实现太慢，用衰减权重卷积近似（窗口=20 足够）
    w = min(20, x.shape[1])
    weights = torch.tensor([smoothing_weight * (1 - smoothing_weight) ** i for i in range(w)],
...
def _ema_simple(x: torch.Tensor, span: int, exact: bool = False) -> torch.Tensor:
    ...
    out[:, 0] = x[:, 0]
    for t in range(1, T):
        out[:, t] = smoothing_weight * x[:, t] + (1 - smoothing_weight) * out[:, t - 1]
    return out
...
def _decay_exp(x: torch.Tensor, d: int, smoothing_weight: float = 0.5) -> torch.Tensor:
    """指数衰减加权平均（近期权重更高）。与 DECAY_LINEAR 对应，平滑更激进。"""
    w = _ts_rolling(x, d)
    weights = torch.tensor([smoothing_weight * (1 - smoothing_weight) ** i for i in range(d)],
