"""
Property-based tests for model_core.engine -- W1ngmanEngine training utilities.
...
Property 14: W1ngman Forward Pass Valid for Any Sequence Length
  For any sequence length T in [1, MAX_FORMULA_LEN],
  W1ngman.forward(idx) must return (logits, value, task_probs)
...
import pytest
from model_core.engine import _build_walk_forward_folds, W1ngmanEngine
...
def test_walk_forward_gap_invariant(T, n_folds, gap):
    ...
    **Validates: Requirements T2.1, T2.3**
    """
    label_horizon = 2
    assume((T - label_horizon - (n_folds - 1) * gap) // n_folds >= 2)
    folds = _build_walk_forward_folds(T, n_folds, gap, label_horizon)
    for fold in folds:
        actual_gap = fold["gap"]
        assert actual_gap == gap
        assert fold["val_start"] == fold["train_end"] + actual_gap
        assert fold["val_start"] < T
        assert fold["val_end"] <= T - label_horizon
...
def _sample_constrained_formula(seed: int, total_steps: int) -> list:
    ...
    validity constraints.  Does NOT depend on an W1ngman model -- this
...
@settings(max_examples=50)
@pytest.mark.skip(reason="The RD-Agent generator does not use the retired policy sampler")
...
@pytest.mark.skip(reason="The retired policy network is kept only for import compatibility")
def test_w1ngman_forward_any_length(seq_len):
    """
    Property 14: W1ngman.forward Valid for Any T in [1, MAX_FORMULA_LEN]

    For any seq_len in [1, 12], W1ngman.forward(idx) must:
...
@settings(max_examples=30)
@pytest.mark.skip(reason="The RD-Agent generator does not restore policy-network snapshots")
