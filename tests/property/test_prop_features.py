...
def test_compute_features_shape_and_nan_free(N: int, T: int):
    ...
    For any N ∈ [1, 10] and T ∈ [21, 200] with positive OHLCV inputs,
        compute_features must return a tensor of shape [N, INPUT_DIM, T] with no NaN/Inf.
    ...
    **Validates: Requirements F1.1, F1.2, F1.10**
    ...
    assert out.shape == (N, expected_dim, T), (
        f"Expected shape ({N}, {expected_dim}, {T}), got {tuple(out.shape)}"
    )
