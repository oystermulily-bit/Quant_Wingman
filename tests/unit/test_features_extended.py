...
class TestComputeFeaturesShape:
    """compute_features 输出形状应为 [N, INPUT_DIM, T]。"""

    def test_output_shape_default(self):
        raw = _make_raw_dict(N=3, T=50)
        out = MT5FeatureEngineer.compute_features(raw)
        expected_dim = MT5FeatureEngineer.INPUT_DIM
        assert out.shape == (3, expected_dim, 50), (
            f"Expected shape (3, {expected_dim}, 50), got {tuple(out.shape)}"
        )
