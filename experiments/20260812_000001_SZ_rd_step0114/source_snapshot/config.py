...
class ModelConfig:
    ...
    #   3. 更大精英池（60）保留更多历史最优
    # 公式生成已切换到 RD-Agent/LLM。一次迭代包含远程提案和本地批量评估，
    # 不再使用旧 W1ngman 的 9000 次 token 采样步。
    BATCH_SIZE      = 16
    TRAIN_STEPS     = int(os.environ.get("RD_AGENT_ROUNDS", "30"))
    MAX_FORMULA_LEN = 10     # 公式长度上限：保持 8（10 会导致 CPU 训练慢 3 倍）

    # ── RD-Agent 公式生成 ───────────────────────────────────────────────
    # 默认值取用户给定区间的中点：token 32~64、深度 5~9、算子 8~16。
    _LEGACY_BACKEND_ENV = "ALPHA" + "MASTER_GENERATOR_BACKEND"
    GENERATOR_BACKEND: str = os.environ.get(
        "W1NGMAN_GENERATOR_BACKEND",
        os.environ.get(_LEGACY_BACKEND_ENV, "rd_agent"),
    )
    RD_AGENT_CANDIDATES_PER_ROUND: int = 16
    RD_AGENT_MAX_TOKENS: int = 64
    RD_AGENT_MAX_DEPTH: int = 9
    RD_AGENT_MAX_OPERATORS: int = 12
    RD_AGENT_FORMULA_TIMEOUT_SECONDS: float = 30.0
    RD_AGENT_ROUND_TIMEOUT_SECONDS: float = 480.0
    RD_AGENT_API_TIMEOUT_SECONDS: float = 180.0
    RD_AGENT_API_RETRIES: int = 3
    RD_AGENT_MODEL: str = os.environ.get(
        "SILICONFLOW_MODEL", "deepseek-ai/DeepSeek-V4-Pro"
    )

    # The suffix is never exposed to formula generation, feedback or selection.
    HOLDOUT_FRACTION: float = 0.15
    HOLDOUT_PROTOCOL: str = "chronological_holdout_15_v1"
    ...
    W1NGMAN_REWARD_WEIGHT:      float = 1.0
