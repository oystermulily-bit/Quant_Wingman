...
def train_from_file(data_file: str, *, from_scratch: bool = False) -> W1ngmanEngine | None:
    ...
    print(
        ...
        f"共{holdout_spec.development_bars}根；最后15% Holdout"
...
def _run_frozen_holdout(engine, full_manager, spec, symbol: str, formula) -> None:
    """Run once after selection; never feed these metrics back into the engine."""
    if formula is None:
        print("  [Holdout] 无冻结公式，未执行独立测试")
        return
    result = evaluate_holdout(full_manager, formula, spec)
    report = save_holdout_report(symbol, formula, result)
    engine.holdout_report = str(report.resolve())
    _attach_holdout_metadata(symbol, formula, engine.holdout_report)
    print(
        f"  [Holdout] 已对冻结公式独立评估一次："
        f"Sharpe={result['sharpe']:.3f} "
        f"累计logPnL={result['total_log_pnl']:.4f}；未用于选优"
    )
    print(f"  [Holdout] 报告: {report}")
