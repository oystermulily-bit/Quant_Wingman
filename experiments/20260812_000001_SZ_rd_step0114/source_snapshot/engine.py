...
class W1ngmanEngine:
    ...
    def _train_rd_agent(
        ...
        *,
        ...
        """RD-Agent propose/evaluate/feedback loop using the existing evaluator.

        Validation, best-strategy selection and backtest semantics intentionally stay
        in quant_w1ngman. The remote model receives vocabulary and structure-only elite
        feedback; all market tensors and all scoring remain local.
        """
        ...
        folds = _build_walk_forward_folds(
            total_bars, self.n_folds, gap=getattr(ModelConfig, "WF_GAP", 20)
        )
        use_wf = len(folds) > 1 and not (
            folds[0]["train_start"] == 0 and folds[0]["train_end"] == total_bars
        )
        ...
        for step in range(start_step, end_step):
            ...
            for result in sorted(results, key=lambda row: row.get("idx", -1)):
                ...
                formula = result["fml"]
                factor = result["res"]
                self._update_elite_pool(validation, formula, step)
                if validation <= self.best_score:
                    continue

                # Keep the existing OOS and exposure gates unchanged.
                if reward > 0.5 and validation < reward * 0.5:
                    continue
                positions = compute_target_positions_stateless(factor)
                exposure = positions.abs().mean().item()
                if exposure < 0.05:
                    continue
                ...
                self._stagnation_steps = 0
                self._update_factor_pool(validation, factor)
                self._save_strategy_live()
                tqdm.write(
                    f"[RD-Agent 新最优 @ 第{step + 1}轮] "
                    f"验证={validation:.3f} 原={old_best:.3f} "
            ...
            tqdm.write(
                ...
                f"最优={self.best_score:.3f} 耗时={round_seconds:.1f}s "
                f"检查点={ckpt}"
            )

        if self.best_formula is None:
            raise RuntimeError("RD-Agent 训练完成，但没有候选公式通过本地验证和风险门控")
