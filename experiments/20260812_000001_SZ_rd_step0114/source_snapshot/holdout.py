...
The development view exposes only the first 85% of the observations to the
research engine.  The final 15% stays in the full data manager and is evaluated
...
HOLDOUT_PROTOCOL = "chronological_holdout_15_v1"
...
class DevelopmentDataView:
    ...
    def __init__(self, full_data_manager, fraction: float = 0.15):
...
def evaluate_holdout(
    ...
    *,
    ...
    factor_full = StackVM().execute(formula, full_data_manager.feat_tensor)
    if factor_full is None:
        raise ValueError("frozen formula could not be executed on full data")
    ...
    return {
        "status": "frozen_formula_evaluated_once",
        "selection_use": False,
