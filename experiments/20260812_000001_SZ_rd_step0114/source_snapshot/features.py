...
class MT5FeatureEngineer:
    ...
    def _ema_simple(x: torch.Tensor, span: int, exact: bool = False) -> torch.Tensor:
        ...
        默认路径（exact=False）：
            ...
            smoothing_weight = 2/(span+1)；有效窗口 w = min(T, ceil(-log(1e-6)/(-log(1-smoothing_weight))))，
            保证尾部权重 (1-smoothing_weight)^w < 1e-6。
            使用首值填充（first-value padding）以匹配递推版初始条件 out[0]=x[0]，
            max|Δ| 与递推版差异实测 < 1e-4。

        可选精确路径（exact=True）：
            严格递推 out[t] = smoothing_weight*x[t] + (1-smoothing_weight)*out[t-1]。
            复杂度 O(N·T)（顺序累积）。
        """
        import math
        ...
        w_full = max(1, math.ceil(-math.log(1e-6) / (-math.log(1.0 - smoothing_weight))))
        ...
        decay = 1.0 - smoothing_weight
        powers = torch.arange(w_full - 1, -1, -1, dtype=x.dtype, device=x.device)
        weights = smoothing_weight * (decay ** powers)                    # 未归一化
