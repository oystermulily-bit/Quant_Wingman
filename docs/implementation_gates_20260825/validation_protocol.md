...
## 2026-09-15：S2版本化适用范围

本文第1、3、4、7、8节保留旧路线的冻结顺序、单信号与周期门槛，继续解释旧结果；不追溯修改D21-v1/v2/v3。用户已批准[阶段4/5联合研究S2](joint_research_s2_20260915.md)：仅对新S2撤销“旧阶段4/原三个信号先通过”的复杂研究前置，不因单因子低IC或未单独胜过基线而拒绝组合研究。

S2每个外层折必须独立在此前数据上执行内层清理、联合筛选、配对/回删及拟合，固定后才输出外层信号；外层标签和得分不回流搜索。最终判断针对完整组合流程的真实扣费表现、统计与集中风险。时点可用性、Purge/Embargo、完整成员日分母、成本和Holdout隔离不撤销。

S2候选清单、日期、`signal_output`、模型及选择规则、统一预算/检验家族、集中风险接受合同未完整冻结，当前只批准流程与工程。外层预测产物不等于交易验证或GO；首版不启用动态公式/RD-Agent。未来公式扩展须在内层生成并与筛选共享预算，不能使用外层或Holdout反馈。
